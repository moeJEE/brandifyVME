...
<head>
    ...
    <meta itemprop="image"
        content="{{ route('home') }}{{ $setting->photo ? '/images/media/' . $setting->photo->file : '/img/200x200.png' }}">
    ...
    <link href="{{ asset('css/front/custom.css') }}" type="text/css" rel="stylesheet">
        ...
        <div class="header__content__venor">
            <div class="header__logo">
                <a href="{{ url('/') }}" title="{{ $setting->title }}">
                    <img width="150" height="35" class="img-fluid logo-front"
            ...
            <form action="{{ route('search') }}" class="header__search__venor" method="GET">
                <input id="search" type="text" name="term"
                    placeholder="{{ clean(trans('niva-backend.search_text'), ['Attr.EnableID' => true]) }}"
                    autocomplete="off">
                    ...
                    @foreach ($menus->sortBy('order') as $prod)
                        @if ($prod->on_off_submenu == 1)
                            <li class="header__nav-item dropdown">
                                <a class="header__nav-link dropdown-toggle" href="{{ $prod->link }}"
                                    role="button" data-toggle="dropdown" aria-haspopup="true"
                                    aria-expanded="false">{{ $prod->name }}
                                </a>
                                {!! $prod->submenu !!}

                            </li>
                        @else
                            <li class="header__nav-item"> <a title="{{ $prod->name }}" class="header__nav-link"
            ...
            <div class="header__actions__venor">
                ...
                <div class="header__lang">
                    ...
                    @if (!empty($currentLang) && count($langs) > 1)
                        ...
                        <a class="header__lang-btn" href="#" role="button" id="dropdownLang"
                            data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <img width="16" height="16"
                                src="{{ $currentLang->photo ? '/images/media/' . $currentLang->photo->file : '/img/200x200.png' }}"
                                alt="flag">
                        ...
                        <ul class="dropdown-menu header__lang-dropdown" aria-labelledby="dropdownLang">
                            @foreach ($langs as $key => $lang)
                                <li><a title="{{ $lang->name }}"
                                        href='{{ route('changeLanguage', $lang->code) }}'><img width="16"
                                            height="16"
                                            src="{{ $lang->photo ? '/images/media/' . $lang->photo->file : '/img/200x200.png' }}"
                                            alt="flag"><span>{{ $lang->name }}</span></a></li>
                            @endforeach
                        </ul>
                    @endif
                ...
                <div class="header__action header__action--signin">
                    <a class="header__action-btn header__action-btn--start-project"
                        href="{{ $headerfooter->sidebar_description }}">
    ...
    @if ($setting->SchmeaORG_switch == 1)
        <div class="hidden" itemscope="" itemtype="https://schema.org/LocalBusiness">
            ...
            <div itemprop="image" itemscope itemtype="http://schema.org/ImageObject">
                <img src="{{ route('home') }}{{ $setting->photo ? '/images/media/' . $setting->photo->file : '/img/200x200.png' }}"
